...
for i in range(1,11):
    ...
    clf_knn = neighbors.KNeighborsClassifier(n_neighbors=7, metric='manhattan')
    ...
    gbdt_b_acc.append(balance5)

    auc_knn = roc_auc_score(y_test, y_knn_pred_t)
    gbdt_auc.append(auc_knn)
    ...
    acc, ses, spc, prc, f1 = metrics(knn_confusion)
    gbdt_acc.append(acc)
    gbdt_ses.append(ses)
    gbdt_spc.append(spc)
    gbdt_prc.append(prc)
